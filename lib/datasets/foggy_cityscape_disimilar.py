...
class foggy_cityscape_disimilar(imdb):
    def __init__(self, image_set, devkit_path=None):
        imdb.__init__(self, 'cs_fg_disimilar_' + '_' + image_set)
        self._year = '2007'
        self._image_set = image_set
        self._devkit_path = cfg_d.FOGGYCITY_disimilar
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        for i, cls in enumerate(self._classes):
            ...
            recs += [rec[0][0]]
            precs += [prec[0][0]]
            print('recall for {} = {}'.format(cls, rec[0][0]))
            print('precsion for {} = {}'.format(cls, prec[0][0]))

            # recs += [np.mean(rec)]
            # precs += [np.mean(prec)]
            # print('recall for {} = {}'.format(cls, np.mean(rec)))
            # print('precsion for {} = {}'.format(cls, np.mean(prec)))
